...
class WGANGP(Model):
    ...
    def build_optimization(self):
        ...
        d_hat = self.real_network.build(x_hat, times=3, reuse=tf.AUTO_REUSE, normal=False)
        gradients = tf.gradients(d_hat, x_hat)[0]
        slopes = tf.sqrt(tf.reduce_sum(tf.square(gradients), reduction_indices=[1]))
        gradient_penalty = 10 * tf.reduce_mean((slopes - 1.0) ** 2)

        self.real_loss_op = tf.reduce_mean(self.fake_output) - tf.reduce_mean(self.real_output) + gradient_penalty
...
if __name__ == '__main__':
    ...
    generator_size = 100
    img_size = [28, 28, 1]
    network = WGANGP("WGANGP", 64, generator_size=generator_size, img_size=img_size)
    network.buind(train_fn=tf_tools.adam_fn, real_lr=1e-4, fake_lr=1e-4)
    network.complete()

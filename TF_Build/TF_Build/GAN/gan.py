...
class GAN(Model):
    def __init__(self, scope_name, channel_min, img_size, generator_size=100, channel_rate=2):
        super(GAN, self).__init__(scope_name)
    ...
    def buind(self, train_fn=tf_tools.adam_fn, real_lr=1e-5, fake_lr=1e-5):
        with tf.variable_scope(self.scope_name) as scope:
            ...
            self.real_train_fn = train_fn(real_lr)
            self.fake_train_fn = train_fn(fake_lr)
    ...
    def buind_network(self, fake_normal=True):
        ...
        real_output =  tf.nn.sigmoid(self.real_network.build(self.input_img, times=3, normal=False))
        fake_output = tf.nn.sigmoid(self.real_network.build(fake_img, times=3, reuse=tf.AUTO_REUSE, normal=False))

        return fake_img, real_output, fake_output
            
    def build_optimization(self):
        self.real_loss_op =  -tf.reduce_mean(tf.log(self.real_output + 1e-12) + tf.log(1. - self.fake_output + 1e-12))
        self.fake_loss_op = -tf.reduce_mean(tf.log(self.fake_output + 1e-12))
...
if __name__ == '__main__':
    ...
    generator_size = 100
    img_size = [28, 28, 1]
    network = GAN("GAN", 64, generator_size=generator_size, img_size=img_size)
    network.buind(train_fn=tf_tools.adam_fn, real_lr=1e-5, fake_lr=1e-4)
    network.complete()

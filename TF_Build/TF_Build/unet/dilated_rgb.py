...
class Dilated(Model):
    def __init__(self, scope_name, channel_min, width, height, channel_rate=2):
        ...
        self.width = width
        self.height = height
        self.input_img = tf.placeholder(tf.float32, shape=[None, height, width, 3], name="input_img")
        self.input_seg = tf.placeholder(tf.float32, shape=[None, height, width, 3], name="input_seg")
...
def minbatch_dataset(data_path, batch_size, start_index):
    ...
    start_index = start_index % 4#len(data_dir)
...
if __name__ == '__main__':
    ...
    for step in range(10000):
        ...
        if step % 10 == 0:
            ...
            tool.array_img_save(pred[0], "./save/" + str(step) + ".tif", normal=)

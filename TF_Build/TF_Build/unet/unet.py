...
class UNet(Model):
    def __init__(self, scope_name, channel_min, width, height, channel_rate=2):
        super(UNet, self).__init__(scope_name)
    ...
    def buind_network(self):
        with tf.variable_scope("encoder"):
            ...
            pool1 = tf_tools.conv2d(cnn1, strides=[1, 2, 2, 1], out_channel=out_channel, activation='relu', normal=True, name='pool1')

            out_channel = out_channel * self.channel_rate
            cnn2 = cnn_block(pool1, out_channel=out_channel, name='cnn2')
            pool2 = tf_tools.conv2d(cnn2, strides=[1, 2, 2, 1], out_channel=out_channel, activation='relu', normal=True, name='pool2')

            out_channel = out_channel * self.channel_rate
            cnn3 = cnn_block(pool2, out_channel=out_channel, name='cnn3')
            pool3 = tf_tools.conv2d(cnn3, strides=[1, 2, 2, 1], out_channel=out_channel, activation='relu', normal=True, name='pool3')

            out_channel = out_channel * self.channel_rate
            cnn4 = cnn_block(pool3, out_channel=out_channel, name='cnn4')
            pool4 = tf_tools.conv2d(cnn4, strides=[1, 2, 2, 1], out_channel=out_channel, activation='relu', normal=True, name='pool4')
        ...
        with tf.variable_scope("decoder"):
            ...
            cnn1 = cnn_block(concat1, out_channel=out_channel, name='cnn1')
            ...
            concat2 = tf.concat([concat_list[1], conv2_1], axis=-1)
            cnn2 = cnn_block(concat2, out_channel=out_channel, name='cnn2')
            ...
            concat3 = tf.concat([concat_list[2], conv3_1], axis=-1)
            cnn3 = cnn_block(concat3, out_channel=out_channel, name='cnn3')
            ...
            concat4 = tf.concat([concat_list[3], conv4_1], axis=-1)
            cnn4 = cnn_block(concat4, out_channel=out_channel, name='cnn4')
